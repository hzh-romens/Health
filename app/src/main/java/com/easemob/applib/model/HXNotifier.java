...
public class HXNotifier {
    ...
    protected final HashSet<String> fromUsers = new HashSet<String>();
     ...
     * @return
     */
    public HXNotifier init() {
        notificationManager = (NotificationManager) MyApplication.applicationContext.getSystemService(Context.NOTIFICATION_SERVICE);

        packageName = MyApplication.applicationContext.getApplicationInfo().packageName;
        ...
        audioManager = (AudioManager) MyApplication.applicationContext.getSystemService(Context.AUDIO_SERVICE);
        vibrator = (Vibrator) MyApplication.applicationContext.getSystemService(Context.VIBRATOR_SERVICE);
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            String appname = (String) packageManager.getApplicationLabel(MyApplication.applicationContext.getApplicationInfo());
            ...
            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(MyApplication.applicationContext)
                    .setSmallIcon(MyApplication.applicationContext.getApplicationInfo().icon)
                    .setWhen(System.currentTimeMillis())
                    .setAutoCancel(true);

            Intent msgIntent = MyApplication.applicationContext.getPackageManager().getLaunchIntentForPackage(packageName);
            if (notificationInfoProvider != null) {
                // 设置自定义的notification点击跳转intent
                msgIntent = notificationInfoProvider.getLaunchIntent(message);
            }

            PendingIntent pendingIntent = PendingIntent.getActivity(MyApplication.applicationContext, notifyID, msgIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public void viberateAndPlayTone(EMMessage message) {
        ...
        HXSDKModel model = IMHXSDKHelper.getInstance().getModel();
        ...
        try {
            ...
            if (model.getSettingMsgSound()) {
                if (ringtone == null) {
                    ...
                    ringtone = RingtoneManager.getRingtone(MyApplication.applicationContext, notificationUri);
                    if (ringtone == null) {
